...
from utils.helpers import fix_seed, compute_nbr_parameters, create_checkpoints_dir, load_model
...
if __name__ == "__main__":
    ...
    if options.net_name.upper() == 'COLORIZATION':
        ...
        if options.continue_training:
            generator=load_model(generator, options.load_ckpt + "/G_it_" + str(options.load_iter) + ".pth", device=hyperparams.device, mode='train', missing_keys=["alpha","beta"])
            discriminator=load_model(discriminator, options.load_ckpt + "/D_it_" + str(options.load_iter) + ".pth", device=hyperparams.device, mode='train')
